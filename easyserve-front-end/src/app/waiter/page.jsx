  ...
  <motion.div key={order.id} variants={itemVariants} className="h-full">
    <Card className="group relative flex h-full min-h-[235px] flex-col justify-between overflow-hidden rounded-2xl border-zinc-200/80 bg-white p-4 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg">
      ...
      <div>
        <div className="mb-3 flex items-start justify-between gap-2">
          <div className="min-w-0">
            <Badge
              variant="outline"
              className={`mb-1 px-2 py-0.5 text-[10px] ${
                ready ? "border-emerald-300 text-emerald-700" : ""
              }`}
            >
        ...
        <div className="mt-2 border-t border-zinc-100 pt-3">
          ...
          <div className="space-y-1.5">
            {order.items?.map((item) => (
              ...
              >
                <span className="truncate pr-2 font-semibold text-zinc-700">
                  {item.menu_item?.name}
                </span>
                <span className="shrink-0 rounded-md bg-zinc-200 px-1.5 py-0.5 text-[10px] font-bold text-zinc-700">
                  x{item.quantity}
                </span>
              </div>
            ))}
      ...
      <button
        ...
        className={`mt-4 flex w-full items-center justify-center gap-1.5 rounded-xl px-3 py-2.5 text-xs font-bold transition-all active:scale-[0.98] ${
          ready
...
export default function WaiterPage() {
  ...
  const { data: dashboardData } = useGetWaiterDashboardQuery(undefined, {
    pollingInterval: 3000,
    refetchOnFocus: true,
  });
  const { data: cashData } = useGetWaiterCashOrdersQuery(undefined, {
    pollingInterval: 3000,
  });
  ...
  return (
    <RoleGuard allowedRoles={["waiter"]}>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-6 sm:px-6">
        <div className="flex flex-col justify-between gap-3 md:flex-row md:items-center">
          <div>
            ...
            <p className="mt-1 text-sm font-medium text-muted-foreground">
              Manage customer orders, table service and cash receipt.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          <Card className="rounded-2xl border-zinc-200/80 p-4 shadow-sm transition-shadow hover:shadow-md">
            <p className="flex items-center gap-1.5 text-xs font-semibold text-zinc-500">
              <Clock className="h-4 w-4 text-amber-500" />
              Pending
            </p>
            <h2 className="mt-1 text-2xl font-black text-zinc-900">{orders.length}</h2>
          </Card>
          <Card className="rounded-2xl border-zinc-200/80 p-4 shadow-sm transition-shadow hover:shadow-md">
            <p className="flex items-center gap-1.5 text-xs font-semibold text-zinc-500">
            ...
            <h2 className="mt-1 text-2xl font-black text-zinc-900">{activeTables}</h2>
          </Card>
          <Card className="rounded-2xl border-zinc-200/80 p-4 shadow-sm transition-shadow hover:shadow-md">
            <p className="flex items-center gap-1.5 text-xs font-semibold text-zinc-500">
            ...
            <h2 className="mt-1 text-2xl font-black text-zinc-900">{cashOrders.length}</h2>
          </Card>
        </div>
          ...
          {cashOrders.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-zinc-200 bg-zinc-50/70 py-9 text-center">
              <p className="text-sm font-medium text-zinc-500">
                No cash payments waiting for collection.
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {cashOrders.map((order) => (
                <Card key={order.id} className="rounded-2xl border-zinc-200/80 p-4 shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-xs font-semibold text-zinc-500">Order #{order.id}</p>
                      <p className="text-lg font-black text-zinc-900">Rs {order.total_price}</p>
                    </div>
                    <Badge className="bg-orange-100 text-orange-800">Cash</Badge>
                  </div>
                  <button
                    onClick={() => handleCash(order.id)}
                    className="mt-3 w-full rounded-xl bg-orange-500 px-3 py-2.5 text-xs font-bold text-white transition-all hover:bg-orange-600 active:scale-[0.98]"
                  >
                    Cash Received
                  </button>
                </Card>
        ...
        <section>
          ...
          {readyOrders.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-zinc-200 bg-zinc-50/70 py-12 text-center">
              <p className="text-sm font-medium text-zinc-500">No ready orders available.</p>
            </div>
          ) : (
            <motion.div
              variants={containerVariants}
