...
function Restaurant() {
  ...
  useEffect(() => {
    ...
    const validateQrTable = async () => {
      ...
      try {
        const response = await validateTableMutation({ restaurant: param.id, table: qrTable }).unwrap();
        if (cancelled) return;
        dispatch(setDineInContext({ ...response, guests: null, name: "", phone: "", sessionToken: response.session?.token ?? null }));
  ...
  useEffect(() => {
    if (!param?.id || !window.location.search.includes("mode=dine-in") || isQrDineIn || dineIn.active) {
      setRestoringDineIn(false);
      return;
    }
    ...
    try {
      ...
      if (saved?.active && saved?.restaurant?.id?.toString() === param.id?.toString() && saved?.table?.number) {
  ...
  const hasDineInMenus = (dineIn?.menus ?? []).some((menu) => (menu?.menu_items?.length ?? 0) > 0);
  const { data } = useGetRestaurantMenusQuery(param.id, { skip: !param.id || (dineIn?.active && hasDineInMenus) });
  ...
  return (
    <section className="min-h-96 bg-gray-50 py-12">
      <div className="mx-auto max-w-7xl px-4">
        ...
        <p className="mb-10 text-center text-gray-500">{restaurant.description || "Enjoy our exquisite cuisine from the comfort of your table"}</p>
        {dineIn.active ? (
          <div className="mb-6 rounded-xl border border-green-200 bg-green-50 p-4">
            <p className="font-semibold text-green-900">🍽️ Dine-In Order</p>
            <p className="text-sm text-green-700">Table #{dineIn.table.number} · Capacity {dineIn.table.capacity}</p>
            <p className="text-sm text-green-700">Guests: {dineIn.guests || "Not set"}</p>
          </div>
        ) : (
          <div className="mb-6 rounded-xl border border-green-200 bg-green-50 p-4"><p className="font-semibold text-green-900">🍽️ Online Order</p></div>
        )}
        {menus?.map((menu) => (
          <div key={menu.id}>
            <h2 className="mb-6 text-2xl font-bold text-green-900">{menu.name}</h2>
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
              {menu.menu_items?.map((item) => <MenuItemCardStable key={item.id} {...item} />)}
            </div>
          </div>
        ))}
        {!menus?.length && <p className="py-10 text-center text-gray-500">Menu is currently unavailable.</p>}
      </div>
