      ...
      <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-500">
            Customer feedback from completed restaurant orders.
          </p>
        </div>
        <button
          type="button"
          ...
          {reviews.map((review) => (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div>
                  <h3 className="font-bold text-zinc-900">Table #{review.table_number ?? "—"}</h3>
                  <p className="mt-0.5 text-xs text-zinc-400">{review.customer_name || "Customer"}</p>
                </div>
              ...
              <p className="mt-4 border-t border-zinc-100 pt-3 text-[11px] font-medium text-zinc-400">
                {review.created_at ? new Date(review.created_at).toLocaleString() : "Recently submitted"}
              </p>
            </motion.article>
          ))}
        </motion.div>
